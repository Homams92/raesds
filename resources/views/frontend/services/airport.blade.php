...
@php
    ...
    // Mapping kategori ke judul yang tampil
    $judulKategori = [
        'wedding' => 'Layanan Wedding',
        'city' => 'Paket Antar Jemput Bandara',
        'tour' => 'Paket Pariwisata',
        // Tambahkan kategori lain kalau ada
    ];

    $breadcrumbKategori = [
        'wedding' => 'Wedding',
        'city' => 'Antar Jemput Bandara',
        'tour' => 'Pariwisata',
    ];

    $category = $serviceDetail->category ?? 'city'; // default city
@endphp
  ...
  <div class="container">
    <div class="row no-gutters slider-text js-fullheight align-items-end justify-content-start">
      <div class="col-md-9 pb-5 text-white">
        <p class="breadcrumbs">
          <span class="mr-2">
            <a href="{{ route('layanan') }}" class="text-white">Layanan <i class="ion-ios-arrow-forward"></i></a>
          </span>
          <span>{{ $breadcrumbKategori[$category] ?? ucfirst($category) }} <i class="ion-ios-arrow-forward"></i></span>
        </p>
        <h1 class="mb-3">{{ $judulKategori[$category] ?? 'Detail Layanan' }}</h1>
      </div>
    </div>
  </div>
</section>
...
<section class="ftco-section bg-light">
  <div class="container">
    <div class="row justify-content-center">
      ...
      {{-- Jika tidak ada detail, tampilkan daftar --}}
      @empty($serviceDetail)
        @forelse ($services as $service)
        <div class="col-md-4 mb-4">
          <div class="car-wrap rounded ftco-animate shadow-sm bg-white">
            <div class="img rounded d-flex align-items-end"
                 style="background-image: url('{{ Storage::url($service->thumbnail) }}'); height: 200px; background-size: cover; background-position: center;">
            </div>
            <div class="text p-3">
              <h2 class="mb-2">
                <a href="{{ route('services.city') }}?id={{ $service->id }}">{{ $service->title }}</a>
              </h2>
              ...
              <div class="d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('services.airport') }}?id={{ $service->id }}" class="btn btn-sm btn-primary">Lihat Detail</a>
              </div>
            </div>
          </div>
        </div>
        @empty
        <div class="col-12 text-center text-muted">Belum ada layanan antar jemput bandara.</div>
        @endforelse
      @endempty
